...
int main(int argc, char** argv)
{
    ...
    cl_mem h_aP = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_ALLOC_HOST_PTR, sizeof(float) * count, NULL, &err);
    checkError(err, "Creating buffer h_aP");

    // Initialize host buffer a
    // Get a mapped pointer
    float * h_data = (float*)clEnqueueMapBuffer(commands, h_aP, CL_TRUE, CL_MAP_WRITE, 0, sizeof(float) * count, 0, NULL, NULL, &err);
    checkError(err, CL_SUCCESS);

    for (int initIdx = 0; initIdx < count; initIdx ++)
    {
        h_data[initIdx] = h_a[initIdx];
    }

    // unmap and make data in the host buffer valid
    err = clEnqueueUnmapMemObject(commands, h_aP, (void*)h_data, 0, NULL, NULL);
    checkError(err, CL_SUCCESS);
    ...
    cl_mem h_bP = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_ALLOC_HOST_PTR, sizeof(float) * count, NULL, &err);
    checkError(err, "Creating buffer h_bP");
    ...
    cl_mem h_cP = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_ALLOC_HOST_PTR, sizeof(float) * count, NULL, &err);
    checkError(err, "Creating buffer h_cP");    
    ...
    // Copy with mapped buffer method...
    h_data = (float*)clEnqueueMapBuffer(commands, h_aP, CL_TRUE, CL_MAP_READ, 0, sizeof(float) * count, 0, NULL, NULL, &err);
    checkError(err, CL_SUCCESS);
    
    err = clEnqueueWriteBuffer(commands, d_a, CL_TRUE, 0, sizeof(float) * count, h_data, 0, NULL, NULL);
    checkError(err, "Copying h_aP to device at d_b");
